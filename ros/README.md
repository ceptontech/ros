...
### Memory Management
- **Static Buffer Allocation**: One buffer per sensor handle
- **Buffer Reuse**: Prevents frequent memory allocation/deallocation
- **Point Reservation**: Pre-allocates space for expected point count

### Asynchronous Operations
- **Non-blocking Publishing**: Uses `std::async` for point cloud publishing
- **Future Coordination**: Ensures publishing order and prevents race conditions
- **Background Status Monitoring**: Separate thread for sensor timeout detection

## Troubleshooting

### Common Issues
- **No point clouds**: Check topic names and enable flags (`output_by_handle`, `output_by_sn`)
- **Sensor timeout**: Verify network connectivity and `expected_sensor_ips`
- **Missing fields**: Check compile-time flags (`WITH_TS_CH_F`, `WITH_POLAR`)
- **Coordinate problems**: Verify ROS coordinate system transformation
- **Replay issues**: Check PCAP file path and replay flags

### Debug Information
- **ROS Logging**: Enable debug logging to see parameter values and status
- **Topic Monitoring**: Use `rostopic list` and `rostopic echo` to verify data flow
- **Status Topics**: Monitor sensor status topic for timeout notifications
- **Network Diagnostics**: Check UDP port 8808 for sensor connectivity

### Performance Tuning
- **Buffer Sizes**: Adjust publisher queue sizes based on data rate
- **Filtering Parameters**: Tune spatial and quality filters for your application
- **Threading**: Monitor CPU usage of status monitoring thread
- **Memory Usage**: Watch for memory growth with multiple sensors