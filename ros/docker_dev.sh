...
REPO_PATH="$(pwd)"
SDK_PATH="$(pwd)/../cepton-sdk-3.0.24.3-dc211d1"
...
echo "Mounting SDK: ${SDK_PATH} -> /catkin_ws/src/cepton-sdk-3.0.24.3-dc211d1"

docker run -it \
    --name ${CONTAINER_NAME} \
    --mount type=bind,source="${REPO_PATH}",target=/catkin_ws/src/cepton_ros \
    --mount type=bind,source="${SDK_PATH}",target=/catkin_ws/src/cepton-sdk-3.0.24.3-dc211d1 \
    --workdir /catkin_ws \
    ${IMAGE_NAME} \
    /bin/bash
