...
CeptonSubscriber::CeptonSubscriber() : Node("cepton_subscriber")
{
  ...
  rclcpp::Parameter pSubscribeCeptonInfo = get_parameter("subscribe_cepton_info");
  if (pSubscribeCeptonInfo.get_type() != rclcpp::ParameterType::PARAMETER_NOT_SET)
...
void CeptonSubscriber::recv_points(const sensor_msgs::msg::PointCloud2::SharedPtr points)
{
  ...
  if (export_to_csv_) {
    struct stat info;
    if (stat(dir.c_str(), &info) != 0 || !(info.st_mode & S_IFDIR)) mkdir(dir.c_str(), S_IRWXU);
  }
  ...
  if (export_to_csv_) {
    ...
    for (int i = 0; i < n; ++i, ++x_iter, ++y_iter, ++z_iter, ++i_iter) {
      s << *x_iter << "," << *y_iter << "," << *z_iter << "," << *i_iter << endl;
    }
  } else {
    for (int i = 0; i < n; ++i, ++x_iter, ++y_iter, ++z_iter, ++i_iter) {
      printf("X: %f\tY: %f\tZ: %f\tIntensity: %f\n", *x_iter, *y_iter, *z_iter, *i_iter);
    }
  }
}
