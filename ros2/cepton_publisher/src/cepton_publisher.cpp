...
void sensorFrameCallback(CeptonSensorHandle handle, int64_t start_timestamp,
  ...
  node->publish_async(handle, stride);
}

void CeptonPublisher::publish_async(CeptonSensorHandle handle, size_t stride) {
  if (pub_fut_.valid()) pub_fut_.wait();
  pub_fut_ = std::async(std::launch::async, [this, handle, stride]() {
    ...
    mod.resize(handle_to_points[handle].size() / stride);
    ...
    auto f_iter    = make_optional_iter<uint8_t>(cloud, "flags");
    auto c_iter    = make_optional_iter<uint8_t>(cloud, "channel_id");
    auto azim_iter = make_optional_iter<float>(cloud, "azimuth");
    auto elev_iter = make_optional_iter<float>(cloud, "elevation");
    ...
    auto set_fields = [&](float x, float y, float z, float i, int32_t t,
                          int32_t t_us, uint8_t f, uint8_t c, float azim,
                          float elev, float dist) {
    ...
    if (handle_to_points.count(handle)) {
      ...
      auto const num_points = ref.size() / stride;
      for (unsigned i = 0; i < num_points; i++) {
        CeptonPoint const &p0 =
            *reinterpret_cast<CeptonPoint const *>(ref.data() + i * stride);
        ...
        const float x = p0.x * 0.005;
        const float y = p0.y * 0.005;
        const float z = p0.z * 0.005;
