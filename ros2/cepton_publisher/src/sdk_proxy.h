...
#include <libcepton_sdk2.h> // DEBUGHERE
...
class SdkProxy : public ProxyLoader {
  ...
  typedef int (*ft_listen)(int aggregation_mode, CeptonPointsExCallback cb,
  ...
  void LoadSymbols() final {
    ...
    Load(fpListen, "CeptonListenFramesEx");
    Load(fpUnlisten, "CeptonUnlistenFramesEx");
 ...
 public:
  ...
  int ListenFrames(int aggregation_mode, CeptonPointsExCallback cb,
                   void* user_data) {
