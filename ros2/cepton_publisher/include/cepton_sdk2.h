...
struct CeptonSensor;        // Defined below
struct CeptonPanicMessage;  // Defined below

/**
 * Add sensor details. This should be only called within a registered parser.
 */
CEPTON_EXPORT int CeptonAddSensorInfo(CeptonSensorHandle h,
                                      const struct CeptonSensor *info);

/**
 * Add points. This should be only called within a registered parser.
 */
CEPTON_EXPORT int CeptonAddPoints(CeptonSensorHandle h, int64_t timestamp,
 ...
 * Add points.
 */
CEPTON_EXPORT int CeptonAddPointsEx(CeptonSensorHandle h, int64_t timestamp,
                                    size_t count,
...
/**
 * Useful for listening panic packet coming from sensor.
 * @param cb Callback.
 */
 ...
 *   CEPTON_ERROR_INVALID_ARGUMENTS
 */
 ...
 *
 * @deprecated Use FrameFifoEx instead
 *
 * @return Error code
 *  CEPTON_ERROR_INVALID_STATE: If frame FIFO is already enabled.
 *  CEPTON_ERROR_INVALID_STATE if existing listeners are using a different
 *     aggregation mode.
 */
CEPTON_EXPORT int CeptonEnableFrameFifo(int aggregationMode, unsigned nFrames);
 ...
 * @return CEPTON_EXPORT
 *  CEPTON_SUCCESS if successful
